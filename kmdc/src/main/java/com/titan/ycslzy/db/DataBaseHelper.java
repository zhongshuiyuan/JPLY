...
import com.titan.ycslzy.dao.DaoMaster;
import com.titan.ycslzy.dao.DaoMaster.DevOpenHelper;
import com.titan.ycslzy.dao.DaoSession;
import com.titan.ycslzy.dao.Jgdc;
import com.titan.ycslzy.dao.JgdcDao;
import com.titan.ycslzy.dao.JgdcDao.Properties;
import com.titan.ycslzy.dao.Ym;
import com.titan.ycslzy.dao.YmDao;
...
public class DataBaseHelper{
	...
	public static List<Map<String, Object>> selectPointGuiji(Context context,
			String sbh, String startTime, String endTime) {
	...
	/**
	 * get not uploades guiji point
	 */
	public static List<Gjpoint> getNotUploadedPoint(Context context, String sbh, final String startTime, String endTime){
		final List<Gjpoint> gjpoints = new ArrayList<>();
		try {
			String databaseName = ResourcesManager.getInstance(context).getDataBase("guiji.sqlite");
			Class.forName("jsqlite.JDBCDriver").newInstance();
			Database db = new jsqlite.Database();
			db.open(databaseName, jsqlite.Constants.SQLITE_OPEN_READWRITE);
			String sql = "select * from point where sbh ='" + sbh
					+ "' and state = '0' and time between datetime('" + startTime
					+ "') and datetime('" + endTime
					+ "') order by datetime(time) desc";
			Callback callback = new Callback() {
				...
				public boolean newrow(String[] strings) {
					...
					Gjpoint gjpoint = new Gjpoint(id,strings[1],strings[2],strings[3],strings[4],strings[5]);
					gjpoints.add(gjpoint);
					return false;
				}
			};
	...
	/**
	 * updata not uploaded guiji point state
	 */
	public static void updataPoint(Context context, List<Gjpoint> gjpoints){
		try {
			String databaseName = ResourcesManager.getInstance(context).getDataBase("guiji.sqlite");
			Class.forName("jsqlite.JDBCDriver").newInstance();
			Database db = new jsqlite.Database();
			...
			for(Gjpoint gjpoint : gjpoints){
				String sql = "updata point set state = '1' where id ='" + gjpoint.getId() +"'";
				db.exec(sql,callback);
			}
