...
import com.otitan.gylyeq.adapter.TcRenderAdapter;
...
public class RenderSetDialog extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageSys.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View arg0) {
                if(iLayerView.getImgTitleLayer().size() > 0){
                    layerTmdSystemImge(iLayerView.getImgTitleLayer());
                }else{
        ...
        dxtSys.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View arg0) {
                if (iLayerView.getDxtTitleLayer().size() == 0) {
                    ToastUtil.setToast(mContext, "图层未加载");
                    return;
                }
                layerTmdSystemImge(iLayerView.getDxtTitleLayer());
            }
        });
     ...
     * 影像图、地形图图层透明度设置
     */
    public void layerTmdSystemImge(final Map<File, TiledLayer> imgTileLayerMap) {
        final Dialog dialog = new Dialog(mContext, R.style.Dialog);
        // 展示图层透明度和图层颜色设置的dialog
        dialog.setContentView(R.layout.title_layer_tmdsystem);
        dialog.setCanceledOnTouchOutside(true);
        ...
        for (File f : imgTileLayerMap.keySet()) {
            ArcGISLocalTiledLayer layer = (ArcGISLocalTiledLayer) imgTileLayerMap.get(f);
            if (layer != null) {
                final String name = new File(layer.getUrl()).getName();
                int tmdtxt = MyApplication.sharedPreferences.getInt(name, 100);
                seekBar.setProgress(tmdtxt);
...
//		});
    ...
    public void showLayerRender(final MyLayer myLayer) {
        ...
        String width = MyApplication.sharedPreferences.getFloat(myLayer.getLayer().getName() + "owidth", 3)+"";
        outlinewidth.setText(width);
    ...
    private void setLayerRenderer(int seekValue, EditText outlinewidth, MyLayer myLayer) {
        ...
        if (myLayer.getLayer().getGeometryType().equals(Geometry.Type.POLYGON)) {
            ...
            if(flag){
                ...
                uniqueValueRenderer.setDefaultSymbol(myLayer.getSymbol());
