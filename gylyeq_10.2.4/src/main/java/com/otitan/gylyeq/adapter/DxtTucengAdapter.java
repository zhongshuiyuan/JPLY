 ...
 * 图层控制 影像数据adapter
 */
public class DxtTucengAdapter extends BaseAdapter {
	List<File> list = null;
	private LayoutInflater inflater = null;
	private HashMap<String, Boolean> isSelected;
	private BaseActivity activity;
	private HashMap<File, TiledLayer> imgTileLayerMap = new HashMap<>();

	public DxtTucengAdapter(BaseActivity activity, List<File> list,
                            HashMap<String, Boolean> isSelected, HashMap<File, TiledLayer> imgmap) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.imgv.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				ArcGISLocalTiledLayer layer = (ArcGISLocalTiledLayer) imgTileLayerMap.get(list.get(position));
				if(layer != null){
