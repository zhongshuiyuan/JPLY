...
import com.titan.ycslzy.activity.StartActivity;
import com.titan.ycslzy.adapter.AttributeAdapter;
import com.titan.ycslzy.adapter.FeatureResultAdapter;
import com.titan.ycslzy.adapter.SearchXdmAdapter;
import com.titan.ycslzy.db.DataBaseHelper;
import com.titan.ycslzy.db.DbHelperService;
...
import com.titan.ycslzy.entity.Station;
import com.titan.ycslzy.entity.XdmSearchHistory;
...
import com.titan.ycslzy.util.CursorUtil;
import com.titan.ycslzy.util.ResourcesManager;
import com.titan.ycslzy.util.SpatiaProUtil;
import com.titan.ycslzy.util.SymbolUtil;
import com.titan.ycslzy.util.SytemUtil;
import com.titan.ycslzy.util.UtilTime;
...
public class BasePresenter {
    ...
    /**添加tpk文件地图*/
    public TiledLayer addTitleLayer(String path){
    ...
    public View loadCalloutPopuwindow(final Point mappoint){
        Point wgsPoint = (Point) GeometryEngine.project(mappoint,iBaseView.getSpatialReference(), SpatiaProUtil.wgs_4326());
        DecimalFormat decimalFormat = new DecimalFormat(".000000");
        ...
        wgslon1.setText(BaseUtil.decimalTodu(wgsPoint.getX()));
        TextView wgslat1 = (TextView) view.findViewById(R.id.calloutpopuwindow_wgs_lat);
        wgslat1.setText(BaseUtil.decimalTodu(wgsPoint.getY()));
        //西安80坐标系
    ...
    public void addGuijiPoint(Point currentPoint) {
        ...
        if (MyApplication.getInstance().hasNetWork()) {// 在线保存
            ...
            state = webservice.upPoint(MyApplication.macAddress,currentPoint.getX() + "", currentPoint.getY() + "",
                    format.format(new Date()), MyApplication.mobileXlh,MyApplication.mobileType);
            if (state.equals("0")) {
                // 录入失败
