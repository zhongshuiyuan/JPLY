...
import com.titan.ycslzy.adapter.ExpandableAdapter;
import com.titan.ycslzy.adapter.ImgTucengAdapter;
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
                ...
                if (isChecked) {
                    ...
                    if(controlView.getDxtTitleLayer() != null && controlView.getDxtTitleLayer().isInitialized()){
                        controlView.getMapView().removeLayer(controlView.getBaseTitleLayer());
                    }
                    if(controlView.getGraphicLayer() != null && controlView.getGraphicLayer().isInitialized()){
                        controlView.getMapView().removeLayer(controlView.getGraphicLayer());
                    }
...
//        gpflag = setExpendHeight(expandableAdapter, tc_exp,gpflag);
//
//        tc_exp.setOnGroupClickListener(new ExpandableListView.OnGroupClickListener() {
//            @Override
...
//        });
    ...
    /**动态设置控件的高度*/
    private boolean setExpendHeight(ExpandableAdapter adapter,ExpandableListView listview,boolean flag){
        ...
        int hegit = (int) (MyApplication.screen.getHeightPixels() * 0.5);

        if(listViewHeight > hegit){
            layoutParams.height = hegit;
        }else{
            if(flag){
                if(listViewHeight > hegit){
                    layoutParams.height = hegit;
    ...
    public void zoomImageLayer() {
        if(imgTileLayerMap.size() >0){
            ...
            for(String key : imgTileLayerMap.keySet()){
                ...
                GeometryEngine.union(geometries, controlView.getBaseTitleLayer().getSpatialReference()).queryEnvelope(env);
            }
        ...
        if (controlView.getBaseTitleLayer().isVisible()) {
            controlView.getMapView().setExtent(controlView.getBaseTitleLayer().getFullExtent());
            controlView.getMapView().invalidate();
        } else {
