...
import com.titan.ycslzy.entity.ScreenTool.Screen;
...
public class ArcGISUtils {
	...
	public static boolean isPerpendicularX(Polyline line) {
		if (checkLegal(line)) {
			return line.getPoint(0).getX() == line.getPoint(1).getX()? true : false;
		} else {
			try {
	...
	public static void DroolMian(Context mContext, Screen screen,GraphicsLayer graphicsLayerLocation, Point point, Polygon polygon) {
		Paint sPaint = new Paint();
		int size = (int)sPaint.getTextSize();
		Drawable color = mContext.getResources().getDrawable(R.drawable.point_pic);
		if (polygon.getPointCount() >= 3) {
			String str = "";
			DecimalFormat decimalFormat = new DecimalFormat(".000");
			if(BaseActivity.spatialReference.isWGS84()){
				Polygon gon =(Polygon) GeometryEngine.project(polygon,SpatiaProUtil.wgs_4326(),SpatiaProUtil.xian_2382());
				str = " "+decimalFormat.format(Math.abs(gon.calculateArea2D())) + "平方米";
			}else{
				str = " "+decimalFormat.format(Math.abs(polygon.calculateArea2D())) + "平方米";
			}
			PictureMarkerSymbol pic = SymbolUtil.TextPicSymobelArea(mContext,str, Color.BLUE, size, SymbolUtil.MODE.BOTTOM);
			TextSymbol textSymbol = new TextSymbol(size,str,Color.BLUE);
			...
			if (polygon.getPathCount() == 0) {
				PictureMarkerSymbol pictureMarkerSymbol = SymbolUtil.TextPicSymobel(mContext, "起点", Color.BLUE, size,SymbolUtil.MODE.BOTTOM);
				TextSymbol textSymbol = new TextSymbol(size,"起点",Color.BLUE);
				textSymbol.setFontFamily("DroidSansFallback.ttf");
	...
	public static void DroolMian(Context mContext, Screen screen,GraphicsLayer graphicsLayerLocation, Polygon polygon, double area) {
		Paint sPaint = new Paint();
		int size = (int)sPaint.getTextSize();
	...
	public static void DroolLine(Context mContext, Screen screen,GraphicsLayer graphicsLayerLocation, Point point, Polyline polyline) {
		DecimalFormat decimalFormat = new DecimalFormat(".000");
		...
		if (polyline.getPointCount() == 0) {
			textSymbol = new TextSymbol(size,"起点",Color.BLUE);
			pic = SymbolUtil.TextPicSymobel(mContext, "起点", Color.BLUE, size,SymbolUtil.MODE.BOTTOM);
		} else {
			...
			if(BaseActivity.spatialReference.isWGS84()){
				Polyline polyline1 =(Polyline) GeometryEngine.project(line,SpatiaProUtil.wgs_4326(),SpatiaProUtil.xian_2382());
				String str ="\n"+ decimalFormat.format(Math.abs(polyline1.calculateLength2D())) + "米";
				textSymbol = new TextSymbol(size,str,Color.BLUE);
				pic = SymbolUtil.TextPicSymobel(mContext, str, Color.BLUE, size,SymbolUtil.MODE.BOTTOM);
			}else{
				String str ="\n"+ decimalFormat.format(Math.abs(line.calculateLength2D())) + "米";
				textSymbol = new TextSymbol(size,str,Color.BLUE);
				pic = SymbolUtil.TextPicSymobel(mContext, str, Color.BLUE, size,SymbolUtil.MODE.BOTTOM);
			}
