...
import com.titan.ycslzy.adapter.TcRenderAdapter;
...
public class RenderSetDialog extends Dialog {
    ...
    public RenderSetDialog(@NonNull Context context) {
        super(context);
    ...
    protected RenderSetDialog(@NonNull Context context, boolean cancelable, @Nullable OnCancelListener cancelListener) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageSys.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if (layerControlPresenter.imgCheckMap.size() > 1) {
                    layerTmdSystemImge(fileList, layerControlPresenter.imgTileLayerMap);
                } else if (iLayerView.getImgTitleLayer() != null && iLayerView.getImgTitleLayer().isInitialized()) {
...
//		});
     ...
     * 展示图层渲染设置
     */
    public void showLayerRender(final MyLayer myLayer) {final Dialog dialog = new Dialog(mContext, R.style.Dialog);
        // 展示图层透明度和图层颜色设置的dialog
        ...
        int oldcolor = MyApplication.sharedPreferences.getInt(myLayer.getLayer().getName() + "tianchongse", R.color.nocolor);
        if(oldcolor != R.color.nocolor && oldcolor !=0){
            tianchongse.setBackgroundColor(oldcolor);
        }else{
            tianchongse.setBackground(mContext.getDrawable(R.drawable.textview_border));
        }
        ...
        outlinewidth.setText(MyApplication.sharedPreferences.getFloat(myLayer.getLayer().getName() + "owidth", 3) + "");
        final TextView bianjiese = (TextView) dialog.findViewById(R.id.bianjiese);
        int bjoldcolor = MyApplication.sharedPreferences.getInt(myLayer.getLayer().getName() + "bianjiese", R.color.nocolor);
        if(bjoldcolor != R.color.nocolor && bjoldcolor != 0){
            bianjiese.setBackgroundColor(bjoldcolor);
        }else{
            bianjiese.setBackground(mContext.getDrawable(R.drawable.textview_border));
        }
