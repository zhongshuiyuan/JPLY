...
import com.titan.ycslzy.adapter.FeatureResultAdapter;
import com.titan.ycslzy.adapter.SetAdapter;
import com.titan.ycslzy.custom.MorePopWindow;
import com.titan.ycslzy.daoImpl.LocationDaoImpl;
import com.titan.ycslzy.dialog.AddAddressDialog;
import com.titan.ycslzy.dialog.AddnewLayerDialog;
import com.titan.ycslzy.dialog.CoordinateDialog;
import com.titan.ycslzy.dialog.EditPhotoDialog;
import com.titan.ycslzy.dialog.FeatureDelDialog;
import com.titan.ycslzy.dialog.GpsSetDialog;
import com.titan.ycslzy.dialog.JjxxsbDialog;
import com.titan.ycslzy.dialog.LayerSelectDialog;
import com.titan.ycslzy.dialog.MergeFeatureDialog;
import com.titan.ycslzy.dialog.RenderSetDialog;
import com.titan.ycslzy.dialog.ResultAreaDialog;
import com.titan.ycslzy.dialog.SettingDialog;
import com.titan.ycslzy.dialog.ShouCangDialog;
import com.titan.ycslzy.dialog.SpaceStatisticsDialog;
import com.titan.ycslzy.dialog.UpdataMobileInfoDialog;
import com.titan.ycslzy.drawTool.DrawEvent;
import com.titan.ycslzy.drawTool.DrawEventListener;
import com.titan.ycslzy.drawTool.DrawTool;
import com.titan.ycslzy.edite.activity.LineEditActivity;
import com.titan.ycslzy.edite.activity.PointEditActivity;
import com.titan.ycslzy.edite.activity.XbEditActivity;
import com.titan.ycslzy.entity.ActionMode;
import com.titan.ycslzy.entity.MyFeture;
import com.titan.ycslzy.entity.MyLayer;
import com.titan.ycslzy.entity.Row;
import com.titan.ycslzy.mview.ILayerView;
import com.titan.ycslzy.mview.INavigatView;
import com.titan.ycslzy.mview.IYzlView;
import com.titan.ycslzy.mview.LayerControlView;
import com.titan.ycslzy.presenter.BasePresenter;
import com.titan.ycslzy.presenter.GpsCollectPresenter;
import com.titan.ycslzy.presenter.LayerControlPresenter;
import com.titan.ycslzy.presenter.LayerLablePresenter;
import com.titan.ycslzy.presenter.NavigationPresenter;
import com.titan.ycslzy.presenter.RepairPresenter;
import com.titan.ycslzy.presenter.StatisticsSpacePresenter;
import com.titan.ycslzy.presenter.TrajectoryPresenter;
import com.titan.ycslzy.presenter.XbqueryPresenter;
import com.titan.ycslzy.service.LiveNetworkMonitor;
import com.titan.ycslzy.service.NetworkMonitor;
import com.titan.ycslzy.util.ArcGISUtils;
import com.titan.ycslzy.util.BaseUtil;
import com.titan.ycslzy.util.BussUtil;
import com.titan.ycslzy.util.ConverterUtil;
import com.titan.ycslzy.util.GDBUtil;
import com.titan.ycslzy.util.NetUtil;
import com.titan.ycslzy.util.PadUtil;
import com.titan.ycslzy.util.SpatiaProUtil;
import com.titan.ycslzy.util.SymbolUtil;
import com.titan.ycslzy.util.SytemUtil;
import com.titan.ycslzy.util.Util;
import com.titan.ycslzy.util.ZoomControlView;
 ...
 * activity基类
 */
public abstract class BaseActivity extends Activity implements LayerSelectDialog.SetOnItemClickListener,
    ...
    public TextView sjtbImgview, dmMangerImgview, personCenter, xbbjImgview, tcxrImgview;
    public TextView gjsearchImgview,xbsearchImgview;
    /*小班编辑RadioButton*/
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new SettingDialog.UpdataThread(mContext).start();
    }
    ...
    public void initData() {
        ...
        Log.d(TAG,"JVM内存信息：" + Util.toMemoryInfo());
    ...
    private void initView() {
        ...
        TextView statisticsView =(TextView) childview.findViewById(R.id.top_kongjiantongji);
        statisticsView.setOnClickListener(this);
        TextView addnewlayerView =(TextView) childview.findViewById(R.id.top_addnewlayer);
        addnewlayerView.setOnClickListener(this);
        TextView jjsbxx =(TextView) childview.findViewById(R.id.top_addjjxx);
        ...
        Util.setBtnGray(mContext,gpspend);
        Util.setBtnGray(mContext,gpsstop);
    }
    ...
    public void addLayer() {
        ...
        NetworkMonitor monitor = new LiveNetworkMonitor(mContext);
        if(monitor.isConnected() && !tiledLayer.isInitialized()){
            String path = mContext.getResources().getString(R.string.tdt_server);
            TiledLayer layer = new ArcGISTiledMapServiceLayer(path);
            mapView.addLayer(layer);
            tiledLayer = layer;

            String zjpath = mContext.getResources().getString(R.string.tdtzj_server);
            TiledLayer zjlayer = new ArcGISTiledMapServiceLayer(zjpath);
            mapView.addLayer(zjlayer);
        }
        ...
        spatialReference = tiledLayer.getDefaultSpatialReference();
        if(spatialReference == null){
            spatialReference = SpatiaProUtil.wgs_4326();
        }
        /*触摸事件*/
            ...
            public void onStatusChanged(Object source, STATUS status) {
                if (source instanceof FeatureLayer) {
                    if (STATUS.LAYER_LOADED == status) {
                        ...
                        ProgressDialogUtil.stopProgressDialog(mContext);
                        ToastUtil.setToast(mContext, "数据加载失败，请检查数据投影和底图投影是否相同");
                    }
                }
            }
        });
...
//                        + decimalFormat.format(currentPoint.getY())
    ...
    public void gpstart(View view) {
        ...
        if (type == 0) {
            ...
            Util.setBtnGray(mContext,gpstart);
            Util.setBtnBlue(mContext,gpspend);
            Util.setBtnBlue(mContext,gpsstop);
...
//                gpsstop.setEnabled(true);
                Util.setBtnBlue(mContext,gpstart);
                Util.setBtnGray(mContext,gpspend);
                Util.setBtnBlue(mContext,gpsstop);
                gps_spend_flag = true;
                return;
    ...
    class MyAsyncTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(final String... params) {
            if (params[0].equals("uplonlat")) {
                /* 保存GPS坐标数据 */
                basePresenter.addGuijiPoint(gpsPoint);
    ...
    public void updataLocation(Point currentPoint) {
        ...
        if (isFirstLoc) {
            ...
            mapView.setScale(1500);
            mapView.setResolution(5.364418029785134E-6);
            mapView.invalidate();
            isFirstLoc = false;
        }
    }
    ...
    public class MyLocationListenner implements BDLocationListener {
        ...
        public void onReceiveLocation(BDLocation location) {
            if (dxtTiledLayer != null && dxtTiledLayer.isInitialized()) {
                NetUtil.openOrcloseWifi(wifiMgr);
            }
            if (location == null)
                return;
            ...
            initGraphicLocation(currentPoint);
    ...
    public Point getPoint(double longitude, double latitude, double altitude,boolean dwflag) {
        ...
        //1090378.360605908, 3536048.2637996837
        Point point;
        if(!spatialReference.isWGS84()){
            point =(Point) GeometryEngine.project(gpsPoint,SpatiaProUtil.wgs_4326(),spatialReference);
        }else{
            point = gpsPoint;
        }
     ...
     * 生成定位的graphic
     */
    public void initGraphicLocation(Point point) {
        ...
        if (locationGraphic == null) {
            ...
            locationGraphic = new Graphic(point, locationMarkerSymbol);
            locationID = graphicsLayer.addGraphic(locationGraphic);
        } else {
            graphicsLayer.updateGraphic(locationID, point);
        }
        Graphic circleGraphic = graphicsLayer.getGraphic(circleID);
        //Point point =(Point) GeometryEngine.project(currentPoint,spatialReference,SpatiaProUtil.xian_2382());
        //Polygon polygon = GeometryEngine.buffer(point, SpatiaProUtil.xian_2382(), 20, null);
        //circlePolygon =(Polygon) GeometryEngine.project(polygon, SpatiaProUtil.xian_2382(),spatialReference);
        if(spatialReference.isWGS84()){
            circlePolygon = GeometryEngine.buffer(point,spatialReference, 0.001, null);
        }else{
            circlePolygon = GeometryEngine.buffer(point,spatialReference, 20, null);
        }
            ...
            if (active && (drawType == POINT || drawType == ENVELOPE
                ...
                switch (drawType) {
                    ...
                    case FREEHAND_POLYLINE:
                        ...
                        if (size == 0) {
                            ...
                            Polyline line =(Polyline) GeometryEngine.project(polyline,SpatiaProUtil.wgs_4326(),SpatiaProUtil.xian_2382());
                            double d1 = line.calculateLength2D();
                            polyline.lineTo(point);
                            Polyline line1 =(Polyline) GeometryEngine.project(polyline,SpatiaProUtil.wgs_4326(),SpatiaProUtil.xian_2382());
                            double d2 = line1.calculateLength2D();
                            double d = d2 - d1;
                            if (d > 6) {
                                polyline_all.lineTo(point);
                            }
                ...
                if (actionMode == ActionMode.MODE_CEMIAN && drawType == FREEHAND_POLYLINE) {
                    ...
                    if (size >= 3) {
                        ...
                        double area = 0;
                        if(spatialReference.isWGS84()){
                            Polygon polygon1 =(Polygon) GeometryEngine.project(polygon,SpatiaProUtil.wgs_4326(),SpatiaProUtil.xian_2382());
                            area = Math.abs(polygon1.calculateArea2D());
                        }else{
                ...
                if (actionMode == ActionMode.MODE_CEMIAN && drawType == FREEHAND_POLYGON) {
                    double area = 0;
                    if(spatialReference.isWGS84()){
                        Polygon polygon =(Polygon) GeometryEngine.project(polygon_all,SpatiaProUtil.wgs_4326(),SpatiaProUtil.xian_2382());
                        area = Math.abs(polygon.calculateArea2D());
                    }else{
...
//		}
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.pop_login:
                ...
                break;
            case R.id.pop_loginout:
                /*退出*/

                break;
            case R.id.pop_recordepic:
                /*现场记录*/
                showXxjjsbDialog();
                break;
    ...
    private void takephotoAndedit() {
        ...
        View login = personCenterPopup.getContentView().findViewById(R.id.pop_login);
        login.setOnClickListener(this);

        View loginout = personCenterPopup.getContentView().findViewById(R.id.pop_loginout);
        loginout.setOnClickListener(this);

        View record = personCenterPopup.getContentView().findViewById(R.id.pop_recordepic);
        ...
        switch (requestCode) {
            case TAKE_PHOTO:// 拍照
                if (resultCode == RESULT_OK) {
                    EditPhotoDialog dialog = new EditPhotoDialog(mContext,R.style.Dialog, mCurrentPhotoPath, currentPoint);
                    dialog.show();
                    personCenterPopup.dismiss();
                }
                break;
            case PICK_PHOTO:
                if(data != null){
                    //图片选择成功
                    JjxxsbDialog.picList.clear();
                    JjxxsbDialog.picList = data.getStringArrayListExtra(PhotoPickerActivity.KEY_RESULT);
    ...
    public void showXxjjsbDialog(){
        JjxxsbDialog jjxxsbDialog = new JjxxsbDialog(BaseActivity.this,R.style.Dialog);
    ...
    public View loadCalloutPoint(Point point, Polyline polyline) {
        ...
        if(spatialReference.isWGS84()){
            Polyline line =(Polyline) GeometryEngine.project(polyline,SpatiaProUtil.wgs_4326(),SpatiaProUtil.xian_2382());
            dd = line.calculateLength2D();
        }else{
            dd = polyline.calculateLength2D();
        }
...
//        }
    ...
    public void showLayerRenderSystem() {
        ...
        BussUtil.setDialogParams(mContext, setDialog, 0.55, 0.6);
    }
    ...
    public void onBackPressed() {
        if(System.currentTimeMillis() - timeMillis < 2000){
            ...
            ToastUtil.setToast(mContext,"再按一次退出");
        }
    }
